...
사용하세요. 이 문서는 맥이나 리눅스 환경을 가정해서 설명합니다.
...
`-n` 옵션으로 빈 자막은 제거했고 `-m '4.5,0.1'` 옵션으로 최소 표시 시간을 4.5초,
...
`SHORT/FIXED BUT SHORT (0.100 -> 4.000)`는 표시 시간이 0.100초여서 기준보다
짧아서 4.5초로 늘리려 했으나 다음 자막과 겹쳐서 기준보다 짧은 4.000초로
늘렸다는 뜻입니다.
...
두 번째, 세 번째 자막은 뜻대로 변경되어 `SHORT/FIXED`가 표시되었습니다.
...
`-m` 옵션으로 수행만 하면 조건과 무관하게 보정합니다.

3번째 자막의 수행 내역이 없는 것은 표시 시간이 1초 이상이어서 보정할 필요가
때문입니다.

만약 수행 내역에 자주 짧은 자막이 연속해서 이어진다면 최소 표시 시간을
